...
export default function ProductPage() {
  ...
  useEffect(() => {
    ...
    if (!apiUrl || !bizId) { setLoading(false); return; }
    fetch(`${apiUrl}/api/businesses/${bizId}/products`)
      ...
      .then((data: unknown) => {
        ...
        setAllProducts((raw as ApiProduct[]).map(mapApiProduct));
      })
      .catch(() => {})
  ...
  const related = product ? allProducts.filter(p => p.blend === product.blend && p.id !== product.id) : [];
    ...
    return (
      <>
        ...
        <div style={{ maxWidth: 600, margin: "6rem auto", textAlign: "center", padding: "0 1.5rem" }}>
          <p style={{ color: "#777", fontFamily: "var(--font-body)" }}>Loading…</p>
        </div>
      </>
    );
  }
  ...
  const blend = BLEND_META[product.blend] ?? BLEND_META.morning;
    ...
    <>
      ...
      <div style={{
        ...
        <Link href="/#shop" style={{ color: "#999", textDecoration: "none" }}>
          {BLEND_NAME[product.blend] ?? product.blend}
        </Link>
        {" / "}
        <span style={{ color: "var(--lhb-black)" }}>{TYPE_LABEL[product.type] ?? product.type}</span>
      </div>
      ...
      <div style={{
        ...
        <div style={{ paddingTop: ".5rem" }}>
          <div style={{
            ...
            {BLEND_NAME[product.blend] ?? product.blend} · {blend.note}
          </div>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: "1rem", marginBottom: ".5rem" }}>
            <span style={{ fontFamily: "var(--font-display)", fontSize: "2rem", fontWeight: 600 }}>
          ...
          <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: ".55rem", marginBottom: "2rem" }}>
            {[
              ...
              `${BLEND_NAME[product.blend] ?? product.blend} — ${blend.label}`,
          ...
          <div style={{ display: "flex", alignItems: "stretch", gap: ".85rem", flexWrap: "wrap" }}>
            <div style={{
              ...
              <button
                ...
                style={{ width: 44, height: 48, border: "none", background: "none", cursor: "pointer", fontSize: "1.1rem", color: "var(--lhb-black)", lineHeight: 1 }}
              >−</button>
              <span style={{ minWidth: 38, textAlign: "center", fontFamily: "var(--font-display)", fontSize: "1rem" }}>
                {qty}
              </span>
              <button
            ...
            <button
              ...
              onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "var(--lhb-black)"; }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "var(--lhb-moss)"; }}
            >
          ...
          {added && (
            <div style={{
              ...
              <span style={{ fontSize: ".86rem", color: "var(--lhb-moss)", fontWeight: 500, fontFamily: "var(--font-body)", display: "flex", alignItems: "center", gap: ".5rem" }}>
                ✓ Added to your cart
              </span>
              <div style={{ display: "flex", gap: ".6rem" }}>
                <Link href="/" style={{ fontSize: ".72rem", letterSpacing: ".1em", textTransform: "uppercase", textDecoration: "none", padding: ".55rem 1rem", color: "#555", border: "1px solid rgba(90,62,30,.3)", fontFamily: "var(--font-body)" }}>
                  Keep Shopping
                </Link>
                <Link href="/checkout" style={{ fontSize: ".72rem", letterSpacing: ".1em", textTransform: "uppercase", textDecoration: "none", padding: ".55rem 1rem", background: "var(--lhb-moss)", color: "var(--lhb-parchment)", fontFamily: "var(--font-body)" }}>
                  Checkout
                </Link>
              </div>
            </div>
      ...
      {related.length > 0 && (
        <section style={{ maxWidth: 1100, margin: "0 auto", padding: "0 1.5rem 5rem" }}>
          <div style={{ borderTop: "1px solid rgba(90,62,30,.15)", paddingTop: "3rem", marginBottom: "1.5rem" }}>
            <h2 style={{ fontFamily: "var(--font-display)", fontSize: "1.8rem", fontWeight: 500, marginBottom: ".4rem" }}>
              Complete the{" "}
              <em style={{ color: "var(--lhb-moss)", fontStyle: "italic" }}>
                {BLEND_NAME[product.blend] ?? product.blend}
              </em>{" "}ritual
            </h2>
            <p style={{ fontSize: ".78rem", color: "#888", fontFamily: "var(--font-body)", letterSpacing: ".04em" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: "1.25rem" }}>
            {related.map((r) => (
              ...
              >
                ...
                />
                <div>
                  <div style={{ fontFamily: "var(--font-display)", fontSize: "1.1rem", color: "var(--lhb-black)", lineHeight: 1.2 }}>
                    {r.name}
                  </div>
                  <div style={{ fontSize: ".74rem", color: "#888", fontFamily: "var(--font-body)", marginTop: ".3rem" }}>
                    {TYPE_LABEL[r.type] ?? r.type} · ${r.price.toFixed(2)}
                  </div>
                </div>
              </Link>
            ))}
      ...
      {/* ── FOOTER ──────────────────────────────────────────────────────── */}
      <footer style={{ background: "var(--lhb-black)", borderTop: "1px solid rgba(200,168,130,.15)", padding: "1.5rem 2rem", textAlign: "center" }}>
        <p style={{ color: "rgba(255,255,255,.35)", fontFamily: "var(--font-body)", fontSize: ".75rem", marginBottom: ".35rem" }}>
